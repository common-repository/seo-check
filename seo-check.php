...
if (version_compare(phpversion(), "5.5", "<")) {
    exit('Sorry, but "eRanker-Plugin" no loger supports php version ' . phpversion());
}
/**/////////////////////////////////////////////////////////////////////////////
        ...
        $seocheck_leadsettings = array(
            ...
            'agents' => array('default' =>
                array(
                    ...
                    'position' => '',
                    'image' => '',
                    'text' => 'Fill the form in order to view the full report',
                    'text_button' => 'Unlock Report Data',
                    'logo' => ''
                )
            ),
...
function load_css_for_widgets() {
    global $post;
    if (is_page('seo-check') === false 
            //|| $post->ID !== NULL
            ) {
        wp_enqueue_style('jqueryuicss', '//code.jquery.com/ui/1.11.4/themes/smoothness/jquery-ui.css');
        wp_enqueue_script('myjqueryUI', '//code.jquery.com/ui/1.11.4/jquery-ui.js', array('jquery'));
        ...
        wp_enqueue_script('er-js-base', plugins_url(basename(dirname(__FILE__)) . '/js/base.js'), array('jquery'), SEOCHECK_VER);
        wp_enqueue_script('circlesjs', plugins_url(basename(dirname(__FILE__)) . '/js/vendor/circles.min.js'), array('jquery'));
        wp_enqueue_style('widgetstyle', plugins_url(basename(dirname(__FILE__)) . '/css/vendor/widget-style.css'));
        wp_enqueue_style('er-css-widget-report', plugins_url(basename(dirname(__FILE__)) . '/css/widget-report.css'), array(), SEOCHECK_VER);
        //wp_enqueue_script('er-js-report', plugins_url(basename(dirname(__FILE__)) . '/js/report.js'), array('d3js', 'gmaps', 'circlesjs','er-bootstrap-js','highstock'), SEOCHECK_VER);
    }
}
...
add_action('wp_enqueue_scripts', 'load_css_for_widgets');
...
function set_seocheck_echoerror($errorcode, $details = '') {
    switch ($errorcode) {
        case 404:
...
function seocheck_getfrontendurl() {
    ...
    if (empty($the_page) || is_null($the_page)) {
        $the_page = get_page_by_path(SEOCHECK_PAGETITLE);
    }

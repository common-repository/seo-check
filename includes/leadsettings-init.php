...
if (isset($_POST) && isset($_POST['seocheck_leadgeneratorsettings']) && !empty($_POST['seocheck_leadgeneratorsettings']) && is_admin() && current_user_can('manage_options')) {
    ...
    if ($erroragent !== TRUE) {
        ...
        for ($i = 0; $i < count($_POST['seocheck_additional_agents_name']); $i++) {
            ...
            // Imported from goRanker needs testing
            $email = (!empty($_POST['seocheck_additional_email'][$i])) ? $_POST['seocheck_additional_email'][$i] : null;
            $company = (!empty($_POST['seocheck_additional_company'][$i])) ? $_POST['seocheck_additional_company'][$i] : null;

            ...
            <tr class="row_even seocheck_bglgray">
                ...
                <?php
                    foreach ($allfactor_list as $key => $value) {
                        ...
                        $factor[$i] = array('key' => $key,
                                                'value'=> $value);
                        $i++;
                    }
                    $factor_number = count($factor);?>
                    <td class="seocheck_nobg first-td">
                        <?php for($a = 0; $a < ceil($factor_number/2);$a++){?>
                            <label class="factor_list_checkbox" title="" for="factor_<?PHP echo $factor[$a]['key']?>">
                                <input id="factor_<?PHP echo $factor[$a]['key']; ?>" type="checkbox" name="factorsGroup[]" <?PHP echo $is_checked ? 'checked="checked"' : '' ?> value="<?PHP echo $factor[$a]['key'] ?>"><?PHP echo ucfirst($factor[$a]['value']->friendly_name) ?>
                            </label>
                        <?php } ?>
                    </td>
                    <td class="seocheck_nobg second-td" >
                            <?php for($a = ceil($factor_number/2); $a < $factor_number; $a++){?>
                                <label class="factor_list_checkbox" title="" for="factor_<?PHP echo $factor[$a]['key']?>">
                                    <input id="factor_<?PHP echo $factor[$a]['key']; ?>" type="checkbox" name="factorsGroup[]" <?PHP echo $is_checked ? 'checked="checked"' : '' ?> value="<?PHP echo $factor[$a]['key'] ?>"><?PHP echo ucfirst($factor[$a]['value']->friendly_name) ?>
                                </label>
                            <?php } ?> 

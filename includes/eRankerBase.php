...
//    const NAME = "eRankerCommons";
    ...
    public static $agentEranker = array('image' => '/content/themes/eranker/img/lead-generator-pop-up-user-dragos-bg.png', 'text' => 'Would you need help fixing any of the above factors?', 'name' => 'Nicholas Radoi', 'position' => '', 'logo' => '/content/themes/eranker/img/logo-green-grey.png', 'referer' => '', 'text_button' => 'Send E-mail');
    ...
    public static function getFactorStatus($value, $fullFactor) {
        if($value === NULL) {
//            if ($fullFactor->id == 'on-google-maps' || $fullFactor->id == 'on-bing-maps' 
//                    || $fullFactor->id == 'structured-data' || $fullFactor->id == 'adwords'
//                    || $fullFactor->id == 'crawlable') {
//                return self::MISSING;
//            }
//            if (stripos($fullFactor->function,'count') !== FALSE && stripos($fullFactor->type,'json') !== FALSE) {
//                return self::GREEN;
//            }
            return self::MISSING;
        }
    ...
    /*
    * Returns text value for FactorObservatory 's test expectation
    */
    public static function getObservatoryExpectation($id){
        $data_arr = array(
            1 => "Content Security Policy (CSP) is implemented and "
                . "without unsafely inline, this includes 'unsafe-inline' or data: "
                . "inside script-src, overly broad sources such as https: inside "
                . "object-src or script-src, or not restricting the sources for " 
                . "object-src or script-src.", 
            2 => "Contribute.json isn't required on websites that don't belong to Mozilla.",
            3 => "Cookies detected and setted with secure flag and with Httponly "
                . "sessions and any prevented transmition by HSTS",
            4 => "Cross Origin Resource Sharing (CORS) file or headers not implemented and without universal access",
            5 => "HTTP Public Key Pinning (HPKP) header can be preloaded or can't "
                . "be implemented without https",
            6 => "Redirects and final destination is an https URL and not to http URL or the are in the HTTP Strict Transport Security (HSTS) preload list",
            7 => "HTTP Strict Transport Security (HSTS) header implemented and set "
                . "to at least six months",
            8 => "Subresource Integrity (SRI) implemented and all external scripts are loaded securely",
            9 => "X-Content-Type-Options header implemented and options setted to nosniff",
            10 => "X-Frame-Options (XFO) header implemented and setted to SAMEORIGIN or DENY",
            11 => "X-XSS-Protection header implemented, enabled and setted to '1; mode=block'"
        );
        return $data_arr[$id];
    }
}

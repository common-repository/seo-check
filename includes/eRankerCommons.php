...
class eRankerCommons extends eRankerBase {
     ...
     */
    public static function getReportHTML($report, $reportScores, $fullfactors, $logged_in = false, $is_pdf = false, $disable_pdf = false, $show_header = TRUE, $show_title = TRUE, $show_category = TRUE,$user_plan='',$seomonitor_text = null) {
        ...
        //begin report        
        $out .= "<div id='erreport'>";
        $out .= self::getReportScoreHTML($report, $reportScores['score'], self::BIG, $disable_pdf, $show_header, $show_title, $show_category,$thumbnailFactor,false,null,$logged_in);
        ...
        if (!empty($factorTree)) {
            //eRanker ToolBox
            if(!isset($_GET['pdf']) && empty($_GET['pdf']) && self::$isPlugin === false 
                    && isset($seomonitor_text['is_user_logged_in']) && !empty($seomonitor_text['is_user_logged_in'])
                    && isset($seomonitor_text['can_user_see']) && !empty($seomonitor_text['can_user_see'])
                ){
                //When a domain don't have already a seo monitor
                $out .= "\r\n";
                $out .= '<div class="ercategory container-eranker-watch">';                
                    $out .= '<div class="ercategoryheadline">';                
                        $out .= '<h2 class="ercategoryname eranker-watch-tooltip" style="border-color: #bdbdcc">';
                            $out .= '<i class="fa fa-binoculars"></i>&nbsp;'.(!empty($seomonitor_text['my_toolbox']) ? $seomonitor_text['my_toolbox'] : 'My ToolBox');
                        $out .= '</h2>';                
                    $out .= '</div><!-- .ercategoryheadline -->';
                    
                    $out .= '<div class="row watch-row">';                    
                        if(empty($report->watch_seomonitor_id)){
                            $out .= '<div class="factor-name col-xs-12 col-sm-12 col-md-12 col-lg-4">';
                                $out .= '<a href="javascript:void(0)" class="btn col-lg-3" id="eranker_watch" title="Create Website Health Monitor" data-url="'. $report->domain .'" data-initial-report="'. $report->id .'">'
                                                . (!empty($seomonitor_text['not_exist']['enable_now']) ? $seomonitor_text['not_exist']['enable_now'] : 'enable now')
                                            .'</a>';
                                $out .= '<h3 class="h3-seo-monitor col-lg-9"><i class="fa fa-clock-o" aria-hidden="true"></i>&nbsp;Website Health Monitor</h3>';                                               
                            $out .= '</div>';

                            $out .= '<div class="factor-data col-xs-12 col-sm-12 col-md-12 col-lg-8 noselect">';                        
                                $out .= '<p class="seo_monitor_description_short">'. (!empty($seomonitor_text['not_exist']['small_description']) ? $seomonitor_text['not_exist']['small_description'] : 'Watch this monitor over time. Enables you to track and compare different versions of your SEO reports in time.') .'</p>';
                            $out .= '</div>';                    
                        }else{                        
                            $out .= '<div class="factor-name col-xs-12 col-sm-12 col-md-4 col-lg-3">';
                                $out .= '<h3 class="h3-seo-monitor"><i class="fa fa-clock-o" aria-hidden="true"></i>&nbsp;Website Health Monitor</h3>';

                                $out .= '<a href="/healthcheck-monitor/'. $report->domain .'/'. $report->watch_seomonitor_id .'" class="eranker_watch" title="Check Website Health Monitor history">';
                                    $out .= '<i class="fa fa-history"></i>&nbsp;';
                                    $out .= (!empty($seomonitor_text['exist']['check_history']) ? $seomonitor_text['exist']['check_history'] : 'check history');
                                $out .='</a>';

                                $out .= '<a href="/healthcheck-monitor/'. $report->domain .'/'. $report->watch_seomonitor_id .'/compare=yes" class="eranker_watch" title="Compare last two reports of this Website Health Monitor">';
                                    $out.= '<i class="fa fa-balance-scale"></i>&nbsp;';
                                    $out .= (!empty($seomonitor_text['exist']['compare_versions']) ? $seomonitor_text['exist']['compare_versions'] : 'compare versions');
                                $out .= '</a>';

                                $out .= '<a href="'. (!empty($seomonitor_text['exist']['string_last_report']) ? $seomonitor_text['exist']['string_last_report'] : 'javascript:void(0)') .'" class="eranker_watch" title="See last report created by this Website Health Monitor">';
                                    $out .= '<i class="fa fa-area-chart"></i>&nbsp;';
                                    $out .= (!empty($seomonitor_text['exist']['see_last_version']) ? $seomonitor_text['exist']['see_last_version'] : 'see last version');
                                $out .= '</a>';
                                if(!empty($seomonitor_text['exist']['is_disabled'])){
                                    $out .= '<span class="watch-span-disabled">'. (!empty($seomonitor_text['exist']['span_disable']) ? $seomonitor_text['exist']['span_disable'] : 'disabled') .'</span>';
                                }

                                $out .= '<a href="javascript:void(0)" class="eranker_watch '. (!empty($seomonitor_text['exist']['is_disabled']) ? 'enable-seo-monitor' : '') .'" id="disable_watch" '
                                        . 'data-monitor-id="'. $report->watch_seomonitor_id .'"'
                                        . ' title="'. (!empty($seomonitor_text['exist']['is_disabled']) ? (!empty($seomonitor_text['exist']['enable']) ? ucfirst($seomonitor_text['exist']['enable']) : ucfirst('enable')) : (!empty($seomonitor_text['exist']['disable']) ? ucfirst($seomonitor_text['exist']['disable']) : ucfirst('disable'))) .' Website Health Monitor">';
                                    if(!empty($seomonitor_text['exist']['is_disabled'])){
                                        $out .= (!empty($seomonitor_text['exist']['enable']) ? $seomonitor_text['exist']['enable'] : 'enable');
                                    }else{
                                        $out .= "<i class=\"fa fa-pause\"></i>&nbsp;".(!empty($seomonitor_text['exist']['disable']) ? $seomonitor_text['exist']['disable'] : 'disable');
                                    }
                                $out .= '</a>';
                            $out .= '</div>';

                            $out .= '<div class="factor-data col-xs-12 col-sm-12 col-md-8 col-lg-9 noselect">';
                                $out .= "<div id='data_seo_monitor_chart' class='hidden' data-value='". (!empty($seomonitor_text['exist']['all_reports_data']) ? json_encode($seomonitor_text['exist']['all_reports_data']) : 'no data found') ."'></div>";
                                $out .= '<div id="seo_monitor_chart"></div>';
                            $out .= '</div>';                        
                        }
                        
                        //be sure this code will appear just in eranker
                        if(self::$isPlugin === FALSE && empty($_GET['mobileapp']) && is_user_logged_in() && !empty($user_plan) && strcasecmp($user_plan,"free") !== 0){
                            $out .= '<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 containerformmonitor">';
                                $out .= '<div class="row">';
                                
                                if(isset($seomonitor_text['normal_monitor']['monitors']) && !empty($seomonitor_text['normal_monitor']['monitors'])){
                                    $out .= '<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 already_monitor"><p>'. $seomonitor_text['normal_monitor']['existent_monitor'] .'</p>';
                                        
                                        $out .= '<table class="table table-condensed table-bordered">'
                                                . '<thead><tr><th>Name</th><th>Link</th><th>Status</th></tr></thead>'
                                                . '<tbody>';
                                            
                                            foreach($seomonitor_text['normal_monitor']['monitors'] as $monitor){
                                                $out .= '<tr>'
                                                        . '<td>'.$monitor->name.'</td>'
                                                        . '<td><a href="'. ((int)$monitor->is_active == 1 ? $seomonitor_text['normal_monitor']['link'] .'/monitor?monitor_id='. $monitor->id : 'javascript:void(0)') .'">'. $seomonitor_text['normal_monitor']['link'] .'/monitor?monitor_id='. $monitor->id .'</a></td>'
                                                        . '<td>'. ((int)$monitor->is_active == 1 ? '<i class="fa fa-check" title="Active"></i>' : '<i class="fa fa-pause" title="Paused"></i>') .'</td>'
                                                        . '</tr>';
                                            }  
                                    ...
                                    $out .='<div class="questionmarkdiv-container hidden-md hidden-sm hidden-xs"><div class="questionmarkdiv">?</div></div>';
                                
                                    $out .= '<div class="insidecontainerform col-xs-12 col-sm-12 col-md-12">';
                                        ...
                                        $out .= '<div class="labeldivreport col-lg-8 col-md-12 col-sm-12 col-xs-12">'. __('Do you want to activate a Status Monitor for this domain?','er') . '</div>';
                                        ...
                                        $out .= '<div class="parentvalidate col-xs-12 col-sm-12 col-md-12 col-lg-4">';
                                            if(is_user_logged_in()){
                                                ...
                                                $out .= '<input type="text" name="emailmonitortocreate" id="emailmonitortocreate plceholder="'.__("Enter your email address").'">';
                                            }                        
                                        $out .= '</div>';
                    ...
                    $out .= '<div class="clearfix col factor-info">'
                            .'<p class="beautifyfactor-info">'. (!empty($seomonitor_text['big_description']) ? $seomonitor_text['big_description']
                                : 'My beautiful lorem ipsum: Lorem ipsum dolor sit amet, ad vel congue populo accommodare, pri te mentitum suavitate. An vim vitae mollis') .'</p>'
                        . '</div>';
        ...
        //Footer Form eRanker        
        if(self::$isPlugin === false && (strcasecmp($user_plan,"free") === 0 || empty($user_plan))){
            $out .= '<div class="row" id="msgleadgenerator">';
            $out .= '</div>';
            
            $out .= '<div class="row leadGeneratorFooterEranker" id="leadGeneratorFooter">';            
            $out .= '<div class="row">';
            $out .= '<div class="col-md-3">';
            $out .= '<div class="toprrightimgemptymodalFooter" style="text-align: center;">';
            $out .= '<img src="'. self::$agentEranker['image'] .'" style="height: 160px; width: 160px;">';
            $out .= '</div>';
            
            if (!empty(self::$agentEranker['logo']) || !empty(self::$agentEranker['name']) || !empty(self::$agentEranker['position'])) {
                $out .= '<div style="">';
                $out .= '<div style="text-align: center; font-size: 16px;">' . self::$agentEranker['name'] . ' <br />' . self::$agentEranker['position'] . '<br /><img src="' . self::$agentEranker['logo'] . '" class="agent-eranker-logo" /> </div>';
                $out .= '</div>';
            }
            //md-3
            $out .= '</div>';
            ...
            $out .= '<input type="hidden" value="'.$report->domain."/$report->id".'" name="reporturl">';
            $out .= '<div class="row">';
            
            if (!empty(self::$agentEranker['text'])){
                $out .= '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">';
                    $out .= ' <h1 class="agent-eranker-h1">' . self::$agentEranker['text'] . ' </h1>';
                $out .= '</div>';
            }
            
            $out .= '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">';
                $out .= '<p class="agent-eranker-p">Webmasters and search engines recommend that all the factors checked in the report are to be implemented correctly and monitored on a regular basis in order to achieve and maintain a high ranking score</p>';
            $out .= '</div>';
            
            $out .= '<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">';
                $out .= '<input id="name_leadgenerator" type="text" class="agent-eranker-input form-control" style="height: 40px!important;" name="name_leadgenerator" placeholder="Name">';
            $out .= '</div>';
            
            $out .= '<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">';
                $out .= '<input id="email_leadgenerator" type="text" class="agent-eranker-input form-control" style="height: 40px!important;"  name="email_leadgenerator" placeholder="E-mail">';
            $out .= '</div>';
            
            $out .= '<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">';
                $out .= '<textarea id="description_leadgenerator" type="text" class="agent-eranker-input form-control" style="height: 40px!important;"  name="description_leadgenerator" placeholder="What can we do to help you..."></textarea>';
            $out .= '</div>';
            
            $out .= '<input id="reporturl" type="hidden" class="agent-eranker-input form-control" style="height: 40px!important;"  name="reporturl" value="' . (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] ? "https" : "http") . "://" . $_SERVER["HTTP_HOST"] . $_SERVER["REQUEST_URI"] . '">';

            $out .= '<input id="lead_domain" type="hidden" name="lead_domain" value="' . $report->domain . '">';
            $out .= '<input id="lead_url" type="hidden" name="lead_url" value="' . $_SERVER['SCRIPT_URI'] . '">';
            
            $out .= '<div class="col-lg-2 col-md-4 col-sm-12 col-xs-12">';
                $out .= ' <button id="saveleadgenerator" type="submit" class="btn btn-primary agent-eranker-btn">' . self::$agentEranker['text_button'] . '</button>';
            $out .= '</div>';
            ...
            $out .= '<script>'
                    . 'if(typeof jQuery !== "undefined"){'
                        .'jQuery(document).ready(function () {urlLeadGenerate = "' . self::$urlLeadGenerator . '";});'
                    . '}else if(typeof $ !== "undefined"){'
                        .'$(document).ready(function () {urlLeadGenerate = "' . self::$urlLeadGenerator . '";});'
                    . '}'
                . '</script>';
        }
...
// Code imported from goRanker needs to be teested
...
//                    $out .= '<button type="button" class="btn btn-primary hideformreportmonitor">'. __('Cancel','er') .'</button>';
        ...
        if ((!isset($_COOKIE['leadgenerated']) || empty($_COOKIE['leadgenerated'])) && self::$useleadgenerator !== FALSE && strcasecmp(self::$layoutLeadGenerator, 'POPUP') === 0) {
            ...
            //do not remove id leadGenerator, is used on custom bootstrap
...
//            $out .= '<label for="phone_leadgenerator">Phone</label>';
            ...
            if (!empty(self::$agent['logo']) || !empty(self::$agent['name']) || !empty(self::$agent['position'])) {
                ...
                //Added from goranker needs testing
                 if(empty(self::$agent['company']))
                {
                  $out .= '<div style="float: left;height: 69px;width: 205px; text-align: center; font-size: 16px;line-height: 1.0em;"><strong>' . self::$agent['name'] . '</strong> <br />' . self::$agent['position'] . '<br /><img src="' . self::$agent['logo'] . '" style="max-height: 50px; max-width: 175px;" /> </div>';
                }else{
                  $out .= '<div style="float: left;height: 69px;width: 205px; text-align: center; font-size: 16px;line-height: 1.0em;"><strong>' . self::$agent['name'] . '</strong> <br />' . self::$agent['position'] . '<br /><p style="text-align:center;font-weight:bold;">' . self::$agent['company'] . '</p> </div>';
                }
                $out .= '</div>';
            }
...
//            $out .= ' </div>';
            ...
            $out .= '</div>';
            $out .= '<style>';
            $out .= '#leadGenerator .modal-content { background: url("' . self::$imgfolder . 'lead-generator-pop-up-main-bg.png") center no-repeat;background-color: transparent!important;}';
            if (!empty(self::$agent['image'])) {
    ...
    public static function getFactorHTML($report, $factor, $score, $is_even = false, $is_loggedin = false, $show_header, $show_title, $show_category, $title = null,$user_plan='') {
        ...
        //var_dump('ddddddddddddddddd',$factor->id,'nnnnnnnnnnnnnnnnnnn',$report->factors_available);
        ...
        $out .= (($is_loggedin && !empty($plan) && strcasecmp($plan,"free") != 0)
                ...
                // 
//                ? '<div class="clearfix col factor-info" style="display:block;">
//                   <p class="beautifyfactor-info">' . stripslashes(html_entity_decode(stripslashes($score['model']['description']))) . '</p>'
//                        . (($is_loggedin && !empty($plan) && strcasecmp($plan,"free") != 0) ?
//                            ((html_entity_decode(stripslashes(self::translate("howtofixfactor", $factor))) === "howtofixfactor") ? '' 
//                                : '<p class="beautifyfactor-info">'.html_entity_decode(stripslashes(self::translate("howtofixfactor", $factor))).'</p>') : '') 
//                            .'</div>'
                //Imported Needs testing
                
                 ? '<div class="clearfix col factor-info" style="display:block;"><p class="beautifyfactor-info">' 
                . stripslashes(html_entity_decode(stripslashes($score['model']['description']))) . '</p>'
                        . (($is_loggedin && !empty($plan) && strcasecmp($plan,"free") != 0) 
                        ? ((html_entity_decode(stripslashes(self::translate("howtofixfactor", $factor))) === "howtofixfactor") 
                        ? '' : '<p class="beautifyfactor-info">'.html_entity_decode(stripslashes(self::translate("howtofixfactor", $factor))).'</p>') : '') 
                .'</div>'
     ...
     */
    public static function getReportScoreHTML($report, $generalscore, $format = self::BIG, $disable_pdf = false, $show_header = TRUE, $show_title = TRUE, $show_category = TRUE, $thumbnailFactor = NULL,$report_out = FALSE,$directly_score=null,$is_user_loggedin=null) {
        $out = "";
...
//            }
            ...
            $generalscore['percentage'] = ((!empty($directly_score) && isset($directly_score) && is_numeric($directly_score)) ? (int)$directly_score :  $generalscore['percentage']);
                    
            $score = (int) $generalscore['percentage'] > 1 ? round($generalscore['percentage']) : "0";
            ...
            if (self::$isPlugin == FALSE && !empty($is_user_loggedin)) {
                $out .= '<a id="update-now" onclick="updateReport(\''. $report->url .'\',\''. $report->specialreport_id .'\',\''.(int)$report->watch_seomonitor_id.'\')">' . $translatedwords['updatenow'] . '</a></span>';
            }
...
//                        $out .= '<div><a download id="download-pdf" onclick="hasSupport()">'. $translatedwords['downloadpdfreport'] .'</a></div>';
            ...
            if(!empty($thumbnailFactor) && (int)$thumbnailFactor['is_active'] === 1){
                if(!isset($thumbnailFactor['data']) || empty($thumbnailFactor['data'])){
                    $thumb_URL = self::$imgfolder . "eranker-squared-full.png";
                }else{                    
...
//            }
            ...
            //if($report_out === false){
                $out .='<p>' . $translatedwords['reportforurl'] . ':</p>'
                    . '<h1>' . $report_url . '</h1>';
         //   } 
            
            $out .= '<ul>'
                . '<li class="col green"><i class="fa fa-check"></i><b class="factor-score">' . $translatedwords['successfullypased'] . '<span>' . $generalscore['factors']['green'] . '</span></b><div class="factorbar" style="width:' . ((int) $score_raw_total > 0 ? ($generalscore['factors']['green'] * 100 / $score_raw_total) : $generalscore['factors']['green'] ). '%"></div></li>'
                . '<li class="col orange"><i class="fa fa-minus"></i><b class="factor-score">' . $translatedwords['roomforimprovement'] . '<span>' . $generalscore['factors']['orange'] . '</span></b><div class="factorbar" style="width:' . ((int) $score_raw_total > 0 ?  ($generalscore['factors']['orange'] * 100 / $score_raw_total) : $generalscore['factors']['orange'] ) . '%"></div></li>'
                . '<li class="col red"><i class="fa fa-times"></i><b class="factor-score">' . $translatedwords['errors'] . '<span>' . ( $generalscore['factors']['red'] + $generalscore['factors']['missing'] ) . '</span></b><div class="factorbar" style="width:' . ((int) $score_raw_total > 0 ? ($generalscore['factors']['red'] * 100 / $score_raw_total) : $generalscore['factors']['red'] ) . '%"></div></li>'
                . '</ul>'

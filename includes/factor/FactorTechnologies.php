    ...
    implements FactorDisplay 
{
    ...
    public static function getDisplay($endModel, $data, $report, $factor) {
        ...
        if (isset($data['applications']) && !empty($data['applications'])) {
            ...
            foreach ($data['applications'] as $group) {
                if (gettype($group) == "array") {
                    $version = isset($group['version']) ? $group['version'] : '';
                    $categories = "";
                    if (!isset($category_array[$group['categories'][0]])) {
                        $category_array[$group['categories'][0]] = [$group['name'] => ['name' => $group['name'], 'icon' => $group['icon'], 'version' => $version]];
                } else {
                        $category_array[$group['categories'][0]][$group['name']] = ['name' => $group['name'], 'icon' => $group['icon'], 'version' => $version];
                }
                }
            ...
            foreach ($category_array as $category => $technologies) {

                $html .= '<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" category="' . $category . '">';
                $html .= '<div class="heading-div"><p class="category-css">' . $category . '</p></div>';

                foreach ($technologies as $technology) {
                    $html .= '<div class="col-lg-4"><p class="cat">' . '<img src="https://www.eranker.com/content/themes/eranker/img/technologies/icons/' . $technology['icon'] . '"  height="24" width="24">' . $technology['name'] . '</p>';

                    if (isset($technology['version']) && $technology['version'] !== "") {
                        $html .= '<p class="desc">' . __('Version', 'er') . ': ' . $technology['version'] . '</p>';
                    }
                    $html .= '</div>';
                }
                $html .= '</div>';
